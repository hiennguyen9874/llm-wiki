...
2. Check every in-scope concept for parseable required metadata, lifecycle and staleness, provenance/footnote joins, unsupported claims, unresolved contradictions, link targets, index agreement, and useful inbound context. This step is complete when every concept has been checked against every applicable rule.
...
A page is orphaned when no concept or index reaches it. A broken link is a retrieval defect even though OKF consumers tolerate it. A missing verification event means unverified; it becomes a warning only when the page presents high-impact claims as settled. Recommendations for new concepts require repeated or central mentions, not every noun.
