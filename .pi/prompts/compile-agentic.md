...
3. Run `.pi/agents/wiki-source-scout.md` for read-only compile reconnaissance over relevant `raw/`, `outputs/`, and existing `wiki/` context.
4. Run `.pi/agents/wiki-ingest-planner.md` on the scout handoff to produce a compile plan: affected sources/outputs, canonical pages to update/create, `related_sources`, contradictions/supersession, overview/index/log updates, and human review items.
5. Parent/orchestrator is the only writer and performs all canonical updates.
6. Run `.pi/agents/wiki-ingest-reviewer.md` on the plan and changed files.
