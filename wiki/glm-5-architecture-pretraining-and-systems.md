...
DSA adaptation starts from the mid-training MLA checkpoint: a 1,000-step indexer warm-up is followed by 20B sparse-adaptation tokens. The released indexer forms weighted sums of ReLU query-key scores and selects up to 2,048 causal token positions. It uses interleaved RoPE and can reuse a prior full indexer layer’s top-k indices on configured `shared` layers; eager/SDPA converts the indices into a sparse additive mask, while compatible kernels consume indices directly.[^glm5-report-2026][^glm5-code-2026]
