...
The resulting dataset contains roughly 1.22M image-text pairs and often multiple region-phrase annotations per image. In-vault, SDA matters less as a standalone benchmark than as the data substrate that enables [[mra]]. The code companion [[source-github-shuyu-xjtu-mra]] confirms that SDA is wired as a pretraining dataset via JSON shards under `data/sda/`, image roots under `images/SDA`, and a region-weighted training branch.

## Relationships
- `uses` [[domain-aware-diffusion]]
- `supports` [[mra]]
- `implemented_in` [[source-github-shuyu-xjtu-mra]]
...
- Notes: Supported by the method design, ablation discussion, and released SDA pretraining code/configuration.
