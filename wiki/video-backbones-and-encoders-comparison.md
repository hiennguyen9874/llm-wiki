...
| Selective state-space model | [VideoMamba](videomamba.md) | Bidirectional selective scans over flattened frame-patch sequences | Isotropic recognition/retrieval backbone | Linear sequence scaling and end-to-end sparse-frame long-video evidence; no persistent memory or hour-level validation |
...
| Scaled masked autoencoder | [VideoMAE V2](videomae-v2.md) | Adds running-cell decoder masking and progressive unlabeled/labeled training | General video backbone | Scales to 1.011B parameters with broad action transfer; mixed-source data governance and training cost are limitations |
...
| VideoMamba | ImageNet-1K initialization for supervised transfer; masked video alignment uses a CLIP ViT-B teacher trained on **400M image-text pairs**; multimodal stages use about **5M–25M** image/video-text pairs | The architecture can train without web-scale data, but its strongest masked and multimodal variants inherit large teacher/corpus scale[^videomamba-paper] |
...
| VideoMAE V2 | Self-supervised on **1.348M** mixed-source clips, then optionally supervised on Kinetics-710 (**658k** train videos) | Scales VideoMAE to 1.011B parameters, but scraped WebVid/Instagram licensing and consent are not sufficiently documented[^videomaev2-paper] |
...
| VideoMamba-M | K400 **83.3%** supervised at 64×384²; SSv2 **71.4%** after CLIP-teacher masked alignment | SSM clip classification and bounded sparse-frame scaling; recipes and input settings differ[^videomamba-paper] |
...
| VideoMAE V2-g | K400 **88.5%** at 16×224² after K710 post-pretraining; ActionFormer THUMOS14 **69.6 mAP** | Scaled masked pretraining and transfer across classification/localization; the 90.0% K400 result uses a larger 64×266² input[^videomaev2-paper] |
