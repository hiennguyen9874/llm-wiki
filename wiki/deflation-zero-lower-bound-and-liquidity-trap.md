---
type: Concept
title: Deflation, the zero lower bound, and liquidity traps
description: How deflation redistributes from borrowers to savers, stalls spending, hits the zero lower bound, and creates liquidity traps that undermine monetary stimulus.
tags: [deflation, zero-lower-bound, liquidity-trap, monetary-policy, debt, great-depression, bernanke, gold-standard]
status: stable
created: 2026-08-22
generated: { by: llm-wiki-agent/1, at: 2026-08-22T15:00:00Z }
sources:
  - id: harford-ch4
    resource: ../raw/TheUndercoverEconomistStrikesBack-TimHarford/007-just-enough-inflation.md
    title: "JUST ENOUGH INFLATION - The Undercover Economist Strikes Back (Tim Harford)"
---

# Deflation, the zero lower bound, and liquidity traps

Deflation — falling prices year after year — makes debts heavier in real terms, encourages hoarding and postponement, and disables conventional monetary policy at the zero lower bound, sometimes leaving the economy in a liquidity trap where printing money alone cannot create inflation[^harford-ch4].

## What deflation does

- Definition: sustained year-on-year falls in the general price level (opposite of inflation)[^harford-ch4].

### Debt-deflation and redistribution

- Mortgages and other nominal debts stay fixed while wages (a price) fall, so repayments consume a growing share of income; food and clothes also fall but the debt does not[^harford-ch4].
- Example: $300,000 mortgage — with modest inflation the monthly payment shrinks relative to rising salary; with deflation the payment looms larger[^harford-ch4].
- Transfer from borrowers to savers: borrowers are more likely to spend (they borrowed to do so) while savers are less likely, so unexpected deflation depresses aggregate demand; widespread repayment strain can imperil banks[^harford-ch4].

### Postponement and hoarding

- As cash buys more tomorrow than today, households postpone nonessential purchases, further depressing demand[^harford-ch4].
- With few borrowers, banks offer low interest; savers withdraw cash to cookie jars/mattresses, removing it from the banking system where it could be lent — still less demand and more deflation[^harford-ch4].

### Sticky-price damned-if-you-do trap

- If prices are sticky and do not fall, goods are too expensive relative to what they should be — demand depressed[^harford-ch4].
- If prices do adjust downward, the expectation of further falls gives everyone an incentive to postpone spending — demand still depressed[^harford-ch4].
- Harford calls this "you're stuck" — the core of the Great Depression of the 1930s, which persisted for years[^harford-ch4].

## Why the gold standard made it worse

- Straightforward cure for deflation is to expand the money supply, but under the gold standard you can print paper but not gold; currencies backed by gold could not expand freely[^harford-ch4].
- During the Great Depression countries left the gold standard one by one, often reluctantly, then printed unbacked paper, expanded money, let prices rise and real wages fall, and rehired workers; recoveries followed in roughly the same order as exits from gold[^harford-ch4].
- Central bank money creation from thin air is a superpower to be used responsibly[^harford-ch4].

## The zero lower bound (ZLB)

- Situation at time of writing (post-2008) for U.S., U.K., Japan and eurozone: nominal interest rates at or near zero in a slump[^harford-ch4].
- Nominal rates cannot fall much below zero because cash pays 0%; few would lend at –1% when mattress cash yields more[^harford-ch4].
- A standard policy rule (Taylor-type) suggested nominal rates should have been around –2% at the depths of the crisis — impossible to achieve, so saving was too high, spending too low, recovery slower than warranted[^harford-ch4].
- Real interest rate = nominal rate – inflation. At zero nominal with 2% inflation, lowest real rate is –2%; with higher inflation, "zero" reaches lower in real terms (e.g., 4% inflation → –4% real). Higher underlying inflation thus gives more room before hitting the floor[^harford-ch4].
- This is the clinching argument for a 3–4% inflation target: a higher buffer makes the ZLB less likely and less binding[^harford-ch4].

## Liquidity traps

- Printing money creates inflation only if people spend it; in a recession with rates at zero, people may hoard new money like the anxious 90-year-old in a cookie jar, because if they wanted to spend they could already borrow for free[^harford-ch4].
- Liquidity trap: central bank prints vast amounts yet no inflationary pressure emerges because extra cash is hoarded; describes early Great Depression and, for decades considered a curiosity, became active research again after 2008[^harford-ch4].

### Expecting future inflation as escape

- In theory a determined central bank can break the trap by making people expect future inflation — "money in your pocket will be worthless" — pulling spending forward before value melts away[^harford-ch4].
- Credibility problem: after decades of waging war on inflation (notably Paul Volcker era), promises to create inflation later lack credibility[^harford-ch4].
- Ben Bernanke in 2002 (as Fed governor) said the logic of the printing press must eventually end deflation; as Fed chair facing a real trap he hesitated until September 2012, when the Fed announced open-ended purchases and "highly accommodative" policy even after recovery — a tame, bureaucratic version of the promised future inflation[^harford-ch4].
- Harford's analogy: like a soft-hearted parent's hollow threat — "when we get home it's straight to bed with no supper! Last warning! I mean it!" — the child (public) does not take it seriously, so spending does not revive[^harford-ch4].
- If the trap is already sprung, printing alone may not suffice; Harford points toward fiscal stimulus (Keynes) as the next lever — bridge to next chapter[^harford-ch4].

## Relationships

- Depends on: [Sticky prices, nominal rigidity, and why money matters](sticky-prices-and-monetary-non-neutrality.md)
- Depends on: [Capitol Hill Babysitting Cooperative recession and scrip monetary policy](capitol-hill-babysitting-cooperative-recession.md)
- Depends on: [Three functions of money as medium, store, and unit of account](functions-of-money-medium-store-unit.md)
- Caused: [Inflation targeting and the case for higher inflation (2% vs 3–4% and NGDP)](inflation-targeting-optimal-inflation-and-ngdp.md)
- Uses: [Hyperinflation and the Brazilian URV ghost currency](hyperinflation-and-brazil-urv-ghost-currency.md)
- Uses: [Money burning, Yap stones, and the nature of fiat money](money-burning-yap-stone-and-fiat-money.md)

[^harford-ch4]: Harford, *The Undercover Economist Strikes Back*, ch. 4 (JUST ENOUGH INFLATION).
