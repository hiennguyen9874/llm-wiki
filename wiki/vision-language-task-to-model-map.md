...
| **Video và audio–language** | BLIP, CoCa, MaMMUT cho video; [ONE-PEACE](one-peace-multimodal-representation-learning.md) cho image/audio/text | BLIP và CoCa chủ yếu ghép frame nên không chứng minh temporal reasoning sâu; MaMMUT thêm sparse spatiotemporal tube tokens rồi fine-tune video. ONE-PEACE là lựa chọn rõ nhất trong wiki cho audio–text retrieval/classification và representation ba modality.[^mammut][^one-peace] |
...
3. **Cần một encoder vừa global vừa local:** ưu tiên SigLIP 2 hoặc TIPSv2; BEiT-3 phù hợp khi chấp nhận task-specific fine-tuning/head.
