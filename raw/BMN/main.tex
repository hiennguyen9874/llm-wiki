...
\def\iccvPaperID{3382} % *** Enter the ICCV Paper ID here
...
%where two dimensions indicate starting time  and temporal duration  separately.
%
...
%Stereo matching is a fundamental computer vision task, which aims to find corresponding pixels from stereo images.
%
...
%
%The procedure of generating $M_C$ using $M_F$ is introduced in section 3.4.
...
%To train BMN, first we need generate boundary label sequence $G_S, G_E \in R^T$ and BM confidence label map $G_M$.
...
%$\varphi=(t_s,t_e,p_{conf},p^s_{t_s},p^e_{t_e})$
...
% explain why we use two types of loss function
...
Thus, we can get candidate proposals set $\Psi_p=\left \{ \varphi_i = (t_s,t_e,p_f) \right \}_{i=1}^{N_p}$, where $p_f$ is used for proposals retrieving during redundant proposals suppression. %In Section xx, we explore the proposal performance with different score fusion strategy.  
...
%{{To verify the effectiveness of LGN, we also adopt normal hard-NMS during experiment in section IV.}}
...
%
%We implement BMN based on  Pytorch [xx].
%
...
%C3D & BSN+SNMS	& {\bf 29.58} & {\bf 37.38} &  {\bf 45.55} & {\bf 54.67} & 59.48   \\
\hline 
\hline
...
%2Stream & BSN+SNMS & {\bf 37.46} & {\bf 46.06} &  {\bf 53.21} & 60.64 &  64.52   \\
\bottomrule
\end{tabular}
...
\begin{figure}[t]
\setlength{\abovecaptionskip}{-0.6cm} %缩小caption和图像之间的距离
...
\begin{table}[tbp]
\setlength{\belowcaptionskip}{-0.5cm} %缩小caption和图像之间的距离
\centering
...
\begin{table}[tbp]
%\setlength{\abovecaptionskip}{-0.cm} %缩小caption和图像之间的距离
%\setlength{\belowcaptionskip}{-0.3cm} %缩小caption和下方文字的距离
\centering
