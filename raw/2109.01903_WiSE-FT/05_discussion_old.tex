...
\label{sec:landscape}
\postsec
Our discussion now focuses on the empirical phenomenon we observe when weight-space ensembling all layers in the network.
...
We begin with a natural generalization of \textit{linear mode connectivity} \cite{frankle2020linear} to the setting where accuracy of the endpoints may differ substantially.
...
\textbf{Definition 1:} Parameters $\theta_0$ and $\theta_1$ exhibit \textit{linear mode connectivity} with respect to $f$ and $\mathcal{D}$ if, for all $\alpha \in [0,1]$, $\mathsf{Acc}_{\mathcal{D}, f}\mleft((1-\alpha)\cdot \theta_0 +  \alpha \cdot \theta_1\mright)\ \geq \ (1-\alpha) \cdot \mathsf{Acc}_{\mathcal{D}, f}\mleft(\theta_0\mright) + \alpha \cdot \mathsf{Acc}_{\mathcal{D}, f}\mleft(\theta_1\mright)$.
Note that when $\mathsf{Acc}_{\mathcal{D}, f}\mleft(\theta_0\mright) = \mathsf{Acc}_{\mathcal{D}, f}\mleft(\theta_1\mright)$, this is equivalent to the original definition \cite{frankle2020linear}.\footnote{The original definition used $\gtrapprox \frac{1}{2}\mleft(\mathsf{Acc}_{\mathcal{D}, f}\mleft(\theta_0\mright) + \mathsf{Acc}_{\mathcal{D}, f}\mleft(\theta_1\mright) \mright)$ and so Definition 1 is not a strict generalization. This original definition is less applicable when endpoint accuracy differs substantially.}

\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{figures/figure_definition_small.pdf}
    \vspace*{-2em}
    \caption{The zero-shot and fine-tuned models exhibit \textit{linear mode connectivity} \cite{frankle2020linear} on ImageNet and the main distribution shifts we consider (Observation 1). Moreover, there exists an $\alpha$ for which WiSE-FT outperforms both the zero-shot and fine-tuned models (Observation 2).}
    \vspace*{-1.5em}
    \label{fig:def}
\end{figure*}

\textbf{Observation 1:} As illustrated in Figure~\ref{fig:def}, we observe linear mode connectivity on ImageNet and the five associated distribution shifts we consider (Section~\ref{sec:expsetup}).

To assist in contextualizing Observation 1, we review related phenomena. Neural networks are nonlinear, hence weight-space ensembles only achieve good performance in exceptional cases---ensembling two randomly initialized networks in weight-space achieves no better accuracy than a random classifier \cite{frankle2020linear}. 
...
We are not the first to observe that when interpolating between models with linear mode connectivity, the accuracy of models along the path may exceed that of either endpoint \cite{izmailov2018averaging, neyshabur2020being, pmlr-v139-wortsman21a}. \citet{neyshabur2020being}
