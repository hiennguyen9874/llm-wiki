...
% video analysis is more computationally
% demanding, even in the case of short clips, which still contain
% hundreds of frames. At the same time, 
...
% In this work, we develop a different architecture, combining a ConvNet trained for still frame classification with a ConvNet trained on the dense video optical flow.
% The two \mbox{ConvNets} form two separate recognition streams (spatial and temporal), which are then combined by late fusion. 
...
% A possible reason could be that stacking of several consecutive frames does not explicitly represent the motion in a video, and the network has to infer it implicitly. 
% Following the advent of large-scale image classification ConvNets~\cite{Krizhevsky12}, similar architectures have been applied to the video data.
% In~\cite{Mnih13}, several consecutive screen-shots of a video game were used as a multi-channel input to a ConvNet.
...
% A common method~\cite{Farneback03}, is to use an eight-parameter global motion model, with the energy obtained by approximating local
% image neighbourhoods with polynomials. 
...
as features.
% In our implementation, fusion is performed by combining the softmax class posteriors from the two nets, so they can be trained and evaluated separately 
% (the details of ConvNet training and testing are given in Sect.~\ref{sec:impl}).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \vspace{-1em}
\begin{figure}[ht]
\centering
...
% \subsection{Spatial ConvNet}
% \label{sec:spatial_net}
% We begin with the description of a spatial stream ConvNet, which operates on individual video frames, effectively performing action recognition from still images.
% The static appearance by itself is a useful clue, since some actions are strongly associated with particular objects. 
% Since a spatial ConvNet is essentially an image classification architecture, we can build upon the recent advances in large-scale image recognition methods~\cite{Krizhevsky12},
% and pre-train the network on a large image classification dataset, such as the ImageNet challenge dataset.
% 
% \noindent\textbf{Architecture.}
% \label{sec:CNN_arch}
% The ConvNet layer configuration is similar to that of~\cite{Zeiler13}, which is the work horse of our recognition framework: with limited changes, it is also used in the temporal ConvNet (\sref{sec:temp_net}).
% The input to the network is a $224 \times 224$ RGB image (a crop from a single video frame). It is then passed through five convolutional and two fully-connected layers, interleaved with 
% rectification (ReLU) non-linearities, max-pooling, and response normalisation (\figref{fig:arch}). The final layer is a softmax classifier. 
% Given a test video, a fixed number of frames are sampled with equal temporal spacing between them, and each class is then scored by averaging its soft-max probabilities over the sampled frames.
% Full details of training and testing are given in~\sref{sec:impl:spatial}.
% 
% As will be shown in~\sref{sec:eval}, action classification from still frames (the spatial recognition stream) is fairly competitive on its own.
% Next, we describe the temporal stream ConvNet. This exploits motion and leads to significantly improved accuracy.
...
%\note{AZ: ** ideally this should be complemented by a visualization of the multiple-frame flow **}
...
% for clarity, the displacement vectors are shown with a $4$-pixel step;
...
% trajectory flow can capture the magnitude of movement (sum up all displacement vectors along the trajectory)
...
% The effect of mean subtraction is illustrated in~\figref{fig:mean_sub}
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{figure}[ht]
% \centering
% % \includegraphics[width=\textwidth]{flow_vis}
% \caption{\textbf{Mean displacement subtraction} cancels out the dominant motion component.} 
% \label{fig:mean_sub}
% \end{figure}
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% constructing the input volume $I_{\tau} \in \Rbb^{w \times h \times 2L}$ for a temporal ConvNet based on the optical flow. 
...
% part of the hand-crafted representations is also generalised by a
% convolutional network, considering that a combination of single
% convolutional and rectification layers can be seen as a variant of
% sparse coding (and a network contains several layers of such kind,
% leading to highly non-linear representations).
...
% The columns of~\figref{fig:conv1} correspond to different filters, the rows -- to the input channels (displacement field components).
...
% ~\cite{Dalal06}). 
% Such filters look similar across different input channels (optical flows), which means that they fire if motion changes at any point of the input.
...
% \note{AZ:** possibly add a schematic figure here **}

% \begin{equation}
% \Lcal = \sum_{
% \end{equation}
% This means, for example, that for a UCF
% video only the UCF classification loss will be "active", because we don't have an HMDB label for that video and can not evaluate it on
% the HMDB classification task. 

% \note{discuss why fine-tuning works worse}
...
% , but has less filters in the conv4 layer (512 \vs 1024) and lower-dimensional full7 layer (2048-D \vs 4096-D).
...
% (we will also evaluate on HMDB-51 in the multi-task learning setting).
...
% We also evaluate our implementation of the ``slow fusion'' architecture of~\cite{Karpathy14}, which consists in applying a ConvNet to a stack of RGB frames (11 frames in our case).
From the results,
...
% Finally, when training from scratch, using a stack of frames as an input outperforms a single-frame ConvNet, but it still far off 
...
% From scratch, stacked & \multirow{2}{*}{-} & \multirow{2}{*}{56.4\%} \\ 
% frames (early fusion~\cite{Karpathy14}) & & \\ \hline
\end{tabular}
\label{tab:spatial}
\end{subtable}
~~~
...
% Unless otherwise noted, the input was obtained from $L=10$ consecutive optical flow fields, resulting in $20$ input channels.
...
% In the supplementary material we discuss class confusion, and visualize the learnt temporal filters.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{table}[ht]
% \vspace*{-2mm}
% \vspace{-0.5em}
...
% Another possible source of improvement is video pre-processing to remove the duplicate
% frames and black borders that occur in the
% datasets in question (UCF-101 and HMDB-51). This might be helpful
% particularly for the spatial ConvNet, which is pre-trained on still images
