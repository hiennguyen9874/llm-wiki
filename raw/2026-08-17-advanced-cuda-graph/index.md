...
source: "https://www.lmsys.org/blog/2026-08-17-advanced-cuda-graph"
domain: "lmsys.org"
language: "en"
description: "CUDA Graphs promise to remove kernel-launch overhead, but getting close to that benefit in a real inference engine requires graphing as much of the workload as possible without sacrificing compatibili..."
word_count: 3581
---
...
- [CUDA Graph in SGLang: the Runner/Backend Split and Flexible Combinations](#cuda-graph-in-sglang-the-runnerbackend-split-and-flexible-combinations)
- [Breakable CUDA Graph: Eager Breaks without a Compiler](#breakable-cuda-graph-eager-breaks-without-a-compiler)
- [Full CUDA Graph for Prefill](#full-cuda-graph-for-prefill)
- [Memory Footprint of CUDA Graphs](#memory-footprint-of-cuda-graphs)
- [BCG as a Standalone Library](#bcg-as-a-standalone-library)
...
- **February 21, 2026: SGLang published the first BCG implementation.** The [initial commit](https://github.com/sgl-project/sglang/commit/571035632b263240e8daba7467dc3472e7884877) of [#19102](https://github.com/sgl-project/sglang/pull/19102) already contained `BreakableCUDAGraph`, an eager-break decorator, and the runtime machinery to end and resume CUDA Graph capture. The PR merged on April 11.
- **April 2026: SGLang extended BCG to prefill.** [#22218](https://github.com/sgl-project/sglang/pull/22218), explicitly based on #19102, built BCG into a compiler-free breakable piecewise backend for prefill and merged on April 24. BCG subsequently became SGLang's default prefill CUDA Graph strategy.
...
At replay time, the recorded graph segments and eager functions run in the same order. During capture the marked function is run once between graph segments, and the tensor it returns is retained as a persistent boundary buffer, so its device address stays fixed; the following segment is captured against that address. On every replay the eager function runs normally and returns a fresh tensor, which BCG copies into the retained buffer, so the next segment reads the updated value from the address it was originally captured with. BCG never inspects or traces the operations inside the eager region: they only need to execute correctly.
