...
Time: 07-27-2026, 22:22:08
Link: https://x.com/waterloo_intern/status/2081762065392541951

Body:
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-1.jpg)

![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-2.jpg)
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-3.jpg)

## **GPT-2**

GPT-2 is a decoder-only architecture:

pythontok_emb = self.transformer.wte(idx) # token embeddings of shape (b, t, n_embd)pos_emb = self.transformer.wpe(pos) # position embeddings of shape (t, n_embd)x = self.transformer.drop(tok_emb + pos_emb)for block in self.transformer.h: x = block(x)x = self.transformer.ln_f(x)logits = self.lm_head(x)return logits

pythontok_emb = self.transformer.wte(idx) # token embeddings of shape (b, t, n_embd)pos_emb = self.transformer.wpe(pos) # position embeddings of shape (t, n_embd)x = self.transformer.drop(tok_emb + pos_emb)for block in self.transformer.h: x = block(x)x = self.transformer.ln_f(x)logits = self.lm_head(x)return logits

The input receives token and positional embeddings:

![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-4.jpg)

Each transformer block, zoomed in, looks like this:

pythonclass Block(nn.Module): def __init__(self, config): super().__init__() self.ln_1 = LayerNorm(config.n_embd, bias=config.bias) self.attn = CausalSelfAttention(config) self.ln_2 = LayerNorm(config.n_embd, bias=config.bias) self.mlp = MLP(config)
def forward(self, x): x = x + self.attn(self.ln_1(x)) x = x + self.mlp(self.ln_2(x)) return x

pythonclass Block(nn.Module): def __init__(self, config): super().__init__() self.ln_1 = LayerNorm(config.n_embd, bias=config.bias) self.attn = CausalSelfAttention(config) self.ln_2 = LayerNorm(config.n_embd, bias=config.bias) self.mlp = MLP(config)
def forward(self, x): x = x + self.attn(self.ln_1(x)) x = x + self.mlp(self.ln_2(x)) return x

![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-5.jpg)

The attention process:

python B, T, C = x.size() # batch size, sequence length, embedding dimensionality (n_embd)
...
# manual implementation of attention att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1))) att = att.masked_fill(self.bias[:,:,:T,:T] == 0, float('-inf')) att = F.softmax(att, dim=-1) att = self.attn_dropout(att) y = att @ v # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs) y = y.transpose(1, 2).contiguous().view(B, T, C) # re-assemble all head outputs side by side
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-6.png)
...
pythonvocab_size: int = 50304 # GPT-2 vocab_size of 50257, padded up to nearest multiple of 64 for efficiencyn_layer: int = 12n_head: int = 12n_embd: int = 768
...
pythondef forward(self, x, mask=None, past_kv=None): # x is b,t,d b,t,d=x.shape
...
# at prefill, q,k,v have shapes b,h,t,d # at decode, shape is b, h, 1, d # so i cat at the t dimension, dim(2)
if past_kv is not None: k_past=past_kv[0] v_past=past_kv[1] k=torch.cat((k_past, k), dim=2) v=torch.cat((v_past, v), dim=2)
scores=(q@k.transpose(-1,-2))/math.sqrt(d_head) if past_kv is None: #we're in prefill and need to mask causal_mask=torch.ones(t,t,dtype=bool, device=q.device) causal_mask=torch.triu(causal_mask, diagonal=1) scores=scores.masked_fill(causal_mask, float('-inf'))
if mask is not None: scores=scores.masked_fill(~mask, float('-inf'))
#get attn (bhtt x bhtd) attn=scores.softmax(-1)#bhtt o=attn@v #bhtd o=o.transpose(1,2).contiguous().view(b,t,d) #b,t,d
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-7.png)

Notice the excessive reads and writes, which this paper replaces with:

pythondef forward(self, x, mask=None, cache=None): # x is b,t,d b,t,d=x.shape
d_head=d//self.num_heads
h=self.num_heads
qkv=self.qkv_proj(x)
q=qkv[:, :, :d].view(b,t,h,d_head).transpose(1,2) k=qkv[:, :, d:2*d].view(b,t,h,d_head).transpose(1,2) v=qkv[:, :, 2*d:].view(b,t,h,d_head).transpose(1,2)
...
k=k.transpose(-1,-2)
q=F.elu(q)+1
S,z=cache if cache is not None else (0.0, 0.0) S=S+k@v
z=z+k

o=q@S #bhtd denom=q@z
o_scaled=o/denom
o_scaled=o_scaled.transpose(1,2).contiguous().view(b,t,d) o_proj=self.o_proj(o_scaled) cache=(S,z)
return o_proj, cache
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-8.png)
...
v_old = k @ S # read the board at this key u = beta * (v - v_old) # the delta: only what's actually new S = S + k.transpose(-1, -2) @ u # same outer-product write as before
o = q @ S # read, no denominator o = o.transpose(1, 2).contiguous().view(b, t, d) return self.o_proj(o), S
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-9.jpg)
...
pythonS = torch.zeros(b, h, dh, dh) if cache is None else cache
outs = []for i in range(t): k_i = k[:, :, i:i+1]
v_i = v[:, :, i:i+1] b_i = beta[:, :, i:i+1] v_old = k_i @ S
u_i = b_i * (v_i - v_old) S = S + k_i.transpose(-1, -2) @ u_i # write outs.append(q[:, :, i:i+1] @ S)
...
pythonS = torch.zeros(b, h, dh, dh) if cache is None else cache
outs = []for i in range(t): q = q[:, :, i:i+1]
k = k[:, :, i:i+1]
v = v[:, :, i:i+1]
S=S_old+k@v
o=q@S #bhtd o=self.norm(o) o=o.transpose(1, 2).contiguous().view(b, t, d)
out=self.o_proj(o) cache=S
outs.append(out)
o = torch.cat(outs, dim=2)
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-10.jpg)
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-11.jpg)

pythonS = torch.zeros(b, h, dh, dh) if cache is None else cache
outs = []for i in range(t//C): q_c = q[:, :, i*C:(i+1)*C]
k_c = k[:, :, i*C:(i+1)*C]
v_c = v[:, :, i*C:(i+1)*C]
o_prev=q_c@S #this is everything up to this block
attn=(q_c@k_c.transpose(-1,-2)).tril() #masked attention o_curr=attn@v_c

o=o_prev+o_curr

S_new=k_c.transpose(-1,-2)@v_c #recurrent attention S=S+S_new
outs.append(o)
o = torch.cat(outs, dim=2)
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-12.jpg)
...
pythonS_t = S_{t-1}(I − β_t k_t k_tᵀ) + β_t v_t k_tᵀ
o_t = S_t q_t
...
pythondef chunk_delta_rule_forward(Q, K, V, beta, C): # L: sequence length, d: head dimension L, d = Q.shape
# chunking Q, K, V = map(lambda x: x.reshape(-1,C,d), [Q, K, V]) beta = beta.reshape(-1, C) K_beta = K * beta.unsqueeze(-1) V_beta = V * beta.unsqueeze(-1)
# compute eq. 10 with vectorized forward substitution for fast inverse T = -(K_beta @ K.t()).tril(-1) for i in range(1, C): T[i, :i] = T[i, :i] + (T[i, :, None] * T[:, :i]).sum(-2)
T += torch.eye(C) W = T @ K_beta
U = T @ V_beta

# chunkwise parallel. Eq. 8-9 S = torch.zeros(d, d) O = torch.empty_like(V)
for i in range(L//C): q_i, k_i, w_i = Q[i], K[i], W[i] u_i = U[i] - w_i @ S # the corrections, all of one chunk o_inter = q_i @ S
A_i = (q_i @ k_i.t()).tril() #qk.t o_intra = A_i @ u_i # attention @ v (with corrections, so u) S += k_i.t() @ u_i # update state with addition O[i] = o_intra + o_inter #update output with flash + recurrent return O.reshape(L, d)
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-13.jpg)
...
pythonS_old=cache
S_new=k@v
# cache=S_old+S_newcache=alpha * S_old + S_new

pythonS_old=cache
S_new=k@v
# cache=S_old+S_newcache=alpha * S_old + S_new

![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-14.png)
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-15.jpg)
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-16.jpg)
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-17.jpg)

The KDA update rule remains similar, but the code now looks more like this:

![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-18.jpg)
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-19.jpg)
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-20.jpg)
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-21.jpg)
...
![Image 1](assets/2026-07-27-from-gpt2-to-kimi-k3/image-22.jpg)
...
pythonV = torch.stack(blocks + [partial_block]) # [N+1, B, T, D]K = norm(V)logits = torch.einsum('d, n b t d -> n b t', proj.weight.squeeze(), K)h = torch.einsum('n b t, n b t d -> b t d', logits.softmax(0), V)return h
