...
\usepackage[safe]{tipa} % type phonetic symbols
...
\def\cvprPaperID{2629} % *** Enter the CVPR Paper ID here
\def\confName{CVPR}
\def\confYear{2022}
...
%\begin{figure*}[t]
%    \centering
%    \includegraphics[width=\textwidth]{figures/results_generalization.pdf}
%    \caption{\textbf{Comparison of the zero-shot method, CoOp and CoCoOp in the base-to-new generalization setting}. For learning-based methods (CoOp and CoCoOp), their prompts are learned from the base classes (16 shots). The results strongly justify the strong generalizability of conditional prompt learning.
%    }
...
%ImageNetV2 is created using ImageNet's protocol but different data sources. ImageNet-Sketch is the sketch version of ImageNet with the same 1,000 classes. ImageNet-A contains adversarially filtered images whereas ImageNet-R is a rendition of the ImageNet classes in diverse image styles, e.g., paintings, cartoons and sculptures.
...
%The detailed per-dataset results are presented in Appendix~A.3.
...
%For detailed per-dataset results, please refer to Appendix~A.1.
...
%More detailed per-dataset results are provided in Appendix~A.2.
