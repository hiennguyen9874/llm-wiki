...
\def\cvprPaperID{10812} % *** Enter the CVPR Paper ID here
\def\confName{CVPR}
\def\confYear{2023}
...
% {\tt\small \{jiangding, yemang\}@whu.edu.cn}\\
...
% introduces additional external tools, such as human parsing and Natural Language ToolKit~\cite{loper2002nltk}, which
...
% To our best knowledge, we are the first work to leverage the MLM task from pre-training stage to facilitate learning for downstream retrieval task.
...
\begin{table*}[ht]
  ...
  \hline
  \midrule
  % GNA-RNN\cite{li2017person}            &G    &CVPR17  &VGG       &LSTM          &19.05             &-              &53.64          &-              &-              \\
  % Dual Path\cite{zheng2020dual}         &G    &TOMM20  &RN50      &RN50          &44.40             &66.26          &75.07          &-              &-              \\
  ...
  \bottomrule
  \end{tabular}}%
  \caption{Performance comparisons with state-of-the-art methods on CUHK-PEDES dataset. Results are ordered based on the Rank-1 accuracy. ``G" and ``L" in ``Type" column stand for global-matching/local-matching method.}
  \label{tab:1}
  \vspace{-4mm}
  \end{table*}
...
% \textbf{Effectiveness of SDM Loss.} 
...
% \textbf{Effectiveness of ID Loss.} 
