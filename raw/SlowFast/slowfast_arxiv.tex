...
\begin{document}
	...
	\begin{abstract}
		...
		We report state-of-the-art accuracy on major video recognition benchmarks, Kinetics, Charades and AVA. Code will be made publicly available in PyTorch.
	\end{abstract}
	%%%%%%%%% BODY TEXT
	...
	\begin{figure}[t]
		\centering
		\vspace{1em}
		\includegraphics[width=1\linewidth]{fig/teaser}
	...
	We  evaluate our method on the Kinetics-400 \cite{Kay2017}, Kinetics-600 \cite{Carreira2018}, Charades \cite{Sigurdsson2016} and AVA \cite{Gu2018} datasets. Our comprehensive ablation experiments 	on Kinetics action classification demonstrate the efficacy contributed by SlowFast.  Importantly, SlowFast networks achieve a new state-of-the-art on all four datasets with significant gains to previous systems in the literature.
	...
	There are recent methods focusing on decomposing the convolutions into separate 2D spatial and 1D temporal filters \cite{Feichtenhofer2016a,Tran2018,Xie2018,Qiu2017}.
...
\paragraph{Kinetics-400.} \tblref{tab:sota:k400} shows the comparison with state-of-the-art results for our SlowFast instantiations using various input samplings ($T$\x$\tau$) and backbones: ResNet-50/101 (R50/101) \cite{He2016} and Nonlocal (NL) \cite{Wang2018}.
	...
	Notably, all our results are substantially better than existing results that are also \emph{without ImageNet pre-training}. In particular, our model (79.8\%) is \textbf{5.9\%} absolutely better than the previous best result of this kind (73.9\%). 
...
%			\multicolumn{1}{c}{} 	& \multicolumn{1}{c}{} & & \scriptsize inference\\
	...
	The SlowFast variants from \tblref{tab:sota:k400} (with different backbones and sample rates) are compared in \figref{fig:slowVsSlowFast} the  with their corresponding Slow-only pathway showing the improvement brought by the Fast pathway. Model capacity is measured for a single clip of 256$^2$ spatial size corresponding to 1$/$30 of the overall inference cost. The Fast pathway is able to consistently improve the performance of the Slow counterpart at comparatively low cost. The next subsection provides a more detailed analysis on Kinetics-400.
	
	\paragraph{Kinetics-600} is relatively new, and existing results are limited. So our goal is mainly to provide results for future reference in  \tblref{tab:sota:k600}.  Note that the Kinetics-600 validation set overlaps with the Kinetics-400 training set \cite{Carreira2018}, so we do \emph{not} pre-train on Kinetics-400. 
	The winning entry \cite{He2018} of the latest ActivityNet Challenge 2018 \cite{Ghanem2018} reports a best single-model, single-modality accuracy of 79.0\%. Our variants show good performance with the best model at 81.8\%.
...
\begin{figure}[t] \vspace{-.5em}
	\centering
	\includegraphics[width=1\linewidth]{fig/train_val_curves_SlowFast}
	\caption{Training procedure on Kinetics for Slow-only (blue) \vs SlowFast (green) network. We show the top-1 training error (dash) and validation error (solid). The curves are single-crop \emph{errors}; the video \emph{accuracy} is 72.6\% \vs 75.6\% (see also \tblref{tab:ablation:fusion}).}
	\label{fig:curves}
	\vspace{-1em}
\end{figure}
%##################################################################################################
...
\paragraph{Training curves.} 	\figref{fig:curves} shows the curves of the Slow-only model (72.6\%) \vs the SlowFast model (75.6\%) during the training procedure on Kinetics-400. We plot the single-crop validation error and training error. It shows that the SlowFast model is consistently better than its Slow-only counterpart \emph{throughout} the entire training. 
	...
	There are 211k training and 57k validation video segments. We follow the standard protocol  \cite{Gu2018} of evaluating on 60 classes (see \figref{fig:ava:class_ap}).
	...
	We extend each 2D RoI at a frame into a 3D RoI by replicating it along the temporal axis, following \cite{Gu2018}. We compute RoI features by RoIAlign \cite{He2017} spatially, and global average pooling temporally. The RoI features are then max-pooled and fed to a per-class, sigmoid-based classifier for multi-label prediction.
	...
	Then, the region proposals for action detection are detected person boxes with a confidence of $>$ 0.9, which has a recall of 91.1\% and a precision of 90.7\% for the person class.
	...
	Ground-truth boxes, and proposals overlapping with ground-truth boxes by IoU $>$ 0.75, are used as the samples for training.
	...
	%##################################################################################################
	\newcommand{\instanceA}[1]{\colorbox{blue!20}{#1}}
	\newcommand{\instanceB}[1]{\colorbox{orange!20}{#1}}
	\newcommand{\instanceC}[1]{\colorbox{yellow!20}{#1}}
	\newcommand{\instanceD}[1]{\colorbox{gray!20}{#1}}
	\newcommand{\instanceE}[1]{\colorbox{lime!20}{#1}}
	\newcommand{\instanceF}[1]{\colorbox{magenta!20}{#1}}
	\newcommand{\instanceG}[1]{\colorbox{teal!20}{#1}}
	
	\newcommand{\resizetag}[1]{\resizebox*{!}{1.96em}{#1}}   % control font size by same *height* resizebox
	\newcommand{\resizetaglarge}[1]{\resizebox*{!}{3.7em}{#1}}   % control font size by same *height* resizebox
	...
	\begin{table}[t!]\vspace{-.5em}
		...
		\begin{tabular}{l|c|c|c|c}
			...
			I3D \cite{Girdhar2018}\quad\ &  &  Kinetics-\textbf{600} &  21.9 & 21.0  \\
			\hline
			\textbf{SlowFast} & & Kinetics-400   & 26.3  & - \\
			\textbf{SlowFast} & & Kinetics-\textbf{600}  & {26.8}  & - \\ 
			\textbf{SlowFast}, +NL & & Kinetics-\textbf{600}  & 27.3  & \textbf{27.1} \\ 
			\textbf{SlowFast}++, +NL &  & Kinetics-\textbf{600}  & \textbf{28.3} & - \\
		\end{tabular}
		\vspace{.3em}
		\caption{\textbf{Comparison with the state-of-the-art on AVA}.  All our variants are based on  $T$\x$\tau$  $=$ 8\x8, R101. 
	...
	The work in \cite{Girdhar2018} pre-trains on the larger Kinetics-600 and achieves 21.9 mAP. For fair comparison, we observe an improvement from 26.3 mAP to 26.8 mAP for using Kinetics-600. Augmenting SlowFast with NL blocks \cite{Wang2018} increases this to 27.3 mAP. By using 3 spatial scales and horizontal flip for testing (SlowFast++, \tblref{tab:ava:sota}), we achieve \textbf{28.3 mAP}, a new state-of-the-art on AVA. 
	
	We train this model on train+val (and by 1.5$\times$ longer) and submit it to the official test server \cite{AVALeaverboard2018}. It achieves \textbf{27.1~mAP} single crop accuracy on the test set. %The 0.7 mAP difference to validation accuracy is likely a natural gap between test and val in this task as it is also observed for \cite{Girdhar2018} in \tblref{tab:ava:sota}. 
	
	We further provide an oracle experiment on ground-truth region proposals resulting in \textbf{35.1 mAP}, suggesting future work could also explore to improve the proposal generation. 
	...
	\begin{table}[t]\centering
		...
		\begin{tabular}{l|x{32}x{10}|x{22}}
			\multicolumn{1}{c|}{ model } & ${T \times \tau}$ & $\omega$   & mAP \\
			\shline
			SlowFast, R-50  & 4\x16 & 8  & 24.2  \\  
			SlowFast, R-50  & 8\x8 & 4  & 24.8  \\ 
			SlowFast, R-101  & 8\x8 & 4  & \textbf{26.3}  \\ 
			%			SlowFast, R-101  & 16\x8 & 4  & \textbf{27.3}  \\ 
		\end{tabular}
		\vspace{.3em}
		\caption{\textbf{More instantiations of SlowFast models on AVA}.}
		\label{tab:ava:more}
		\vspace{-1.em}
	\end{table}
	%##################################################################################################
	...
	\tblref{tab:ava:more} shows more SlowFast instantiations. Their performance is consistent with the Kinetics action classification accuracy (see \tblref{tab:sota:k400}), suggesting that our models are robust.
	
	
	\paragraph{Visualization.} 
	We show qualitative results of the SlowFast model in \figref{fig:ava_predictions}.
	Overall, even given some discrepancy between our predictions and ground-truth, our results are visually reasonable. Predictions and ground-truth labels are in several cases extending each other, illustrating the difficulty of this dataset. For example, consider the sequence in the last row of \figref{fig:ava_predictions}. The SlowFast detector is able to predict the right person's reading action with a score of 0.54, but is penalized as this label is not present in the annotations. 
...
We use the initialization method in \cite{He2015}.
	We train with Batch Normalization (BN) \cite{Ioffe2015}, and the BN statistics are computed within each 8 clips.
