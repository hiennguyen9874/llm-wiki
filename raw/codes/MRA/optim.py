...
def create_optimizer(args, model):
    ...
    optimizer_grouped_parameters = [
        ...
        {"params": [], "weight_decay": wd, "lr": lr * lr_x},
        {"params": [], "weight_decay": 0.0, "lr": lr * lr_x}
    ]
    ...
    if hasattr(model, 'init_params'):
        ...
        print("### model has 'init_params', ", len(large_lr), flush=True)
    else:
        large_lr = {}

    if hasattr(model, 'x_params'):
        x_lr = model.x_params
        print("### model has 'x_params', ", len(x_lr), flush=True)
    else:
        x_lr = {}
