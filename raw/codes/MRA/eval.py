...
@torch.no_grad()
def evaluation_itm(model, device, config, args, sims_matrix, image_embeds, text_embeds, text_atts, himage_embeds=None):
    model.eval()
    ...
    if himage_embeds is not None:
        print('### itm_hflip')
        hscore_matrix_t2i = torch.full(sims_matrix.size(), 1000.0).to(device)
    ...
    for i, sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 500, header)):
        ...
        output = model.get_cross_embeds(encoder_output, encoder_att,
                                        text_embeds[start + i].repeat(config['k_test'], 1, 1),
                                        text_atts[start + i].repeat(config['k_test'], 1))[:, 0, :]
        score = model.itm_head(output)[:, 1]
        score_matrix_t2i[start + i, topk_idx] = score

        if himage_embeds is not None:
            encoder_output = himage_embeds[topk_idx]
            output = model.get_cross_embeds(encoder_output, encoder_att,
                                            text_embeds[start + i].repeat(config['k_test'], 1, 1),
                                            text_atts[start + i].repeat(config['k_test'], 1))[:, 0, :]
            score = model.itm_head(output)[:, 1]
            hscore_matrix_t2i[start + i, topk_idx] = score
    ...
    if himage_embeds is not None:
        min_values, _ = torch.min(hscore_matrix_t2i, dim=1)
        replacement_tensor = min_values.view(-1, 1).expand(-1, hscore_matrix_t2i.size(1))
        for i in range(sims_matrix.size(0)):
            hscore_matrix_t2i[i][hscore_matrix_t2i[i] == 1000.0] = replacement_tensor[i][hscore_matrix_t2i[i] == 1000.0]
        hscore_matrix_t2i[hscore_matrix_t2i == 1000.0] = replacement_tensor[hscore_matrix_t2i == 1000.0]
        hscore_matrix_t2i = (hscore_matrix_t2i - hscore_matrix_t2i.min()) / (hscore_matrix_t2i.max() - hscore_matrix_t2i.min())
    ...
    if himage_embeds is None:
        ...
        score_matrix_t2i =( score_matrix_t2i + hscore_matrix_t2i) / 2 + 0.002 * score_sim_t2i
