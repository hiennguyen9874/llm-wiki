...
from .webreid import WebREID
from .MALS import MALS_structure
from .luperson_t import LUPersonT
from .plip import PLIP
import os

__factory = {'CUHK-PEDES': CUHKPEDES, 'ICFG-PEDES': ICFGPEDES, 'RSTPReid': RSTPReid,
            'LuPerson_PEDES':LuPerson_PEDES,'webperson':WebREID, 
            "MALS":MALS_structure,"LUPerson-T":LUPersonT,"PLIP":PLIP}
...
def build_zero_shot_loader(args, finetune=False):
    ...
    if finetune:
        ...
        # syn_dataset = __factory[args.pretrain](root=args.root_dir)
        syn_dataset = __factory[args.pretrain](args=args,root=args.root_dir)
    train_set = ImageTextMLMDataset(syn_dataset.train,
                            train_transforms,
                            text_length=args.text_length)
    num_classes = len(syn_dataset.train)
    ...
    logger.info('using random sampler')
    if args.distributed:
        sampler = DistributedSampler(train_set,num_replicas=int(os.environ['WORLD_SIZE']),rank=int(os.environ["LOCAL_RANK"]),shuffle=True)
...
def build_filter_loader(args, dataset,epoch):
    logger = logging.getLogger("IRRA.dataset")
