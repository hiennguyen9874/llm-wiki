...
import datetime
def plot_distribution(data, range:tuple ,title, x_label, y_label, filename):
        plt.figure(figsize=(10, 5))
        plt.hist(data, bins=100,range=range ,color='blue', alpha=0.7)
        plt.title(title)
        plt.xlabel(x_label)
        plt.ylabel(y_label)
        plt.grid(True)
...
class LuPerson_PEDES(BaseDataset):
    ...
    def __init__(self,args='', root='', verbose=True):
        ...
        self.dataset_dir = '/mnt/tianluzheng/PersonReID_dataset/LUPerson'
        self.image_dir = op.join(self.dataset_dir, 'extracted_images_lu')
        self.caption_dir = '/mnt/tianluzheng/reid/checkpoints'#self.dataset_dir
        ...
        for filename in os.listdir(self.image_dir): # part1234
            ...
            if filename.endswith('.jpg'):
                self.train_img_paths.append(image_path)#len(self.train_img_paths)=761757
        ...
        train_cap_dict = self._merged_multi_json_file(self.train_cap_paths)#1020022
    ...
    def _merged_multi_json_file(self, json_path_list):
        ...
        return merged_dict #len(merged_dict)=1020022 一张图片对应3-4个文本
    ...
    def _get_dataset(self, img_paths, cap_dict):
        ...
        with open('/mnt/tianluzheng/reid/checkpoints/Td-qwen.json', 'r') as json_file:
            data = json.load(json_file)
            for k,v in data.items():
                img_name = k.split('/')[-1]
                safe_dict[img_name].append(v) #1017217
        
        with open('/mnt/tianluzheng/reid/checkpoints/Td-shikra.json', 'r') as json_file:
            data = json.load(json_file)
            for k,v in data.items():
                img_name = k.split('/')[-1]
                safe_dict[img_name].append(v) #1020022
        pid_container = set()
    ...
    def _get_dataset1(self, img_paths, cap_dict):
        safe_dict = collections.defaultdict(list)
        ...
        for i in tqdm(range(len(img_paths))):
            ...
            pid_count += 1
        assert idx_count == len(dataset)
        # plot_distribution(aspect_ratios,(0.1,0.7) ,'Distribution of Image Aspect Ratios', 'Aspect Ratio', 'Frequency', f'/home/face/kaichengyang/tianluzheng/inference_llm/data_attribute/luperson/train_aspect_ratio_distribution_{timestamp}.png')
        # plot_distribution(resolutions,(0,20000) ,'Distribution of Image Resolutions', 'Resolution (pixels)', 'Frequency', f'/home/face/kaichengyang/tianluzheng/inference_llm/data_attribute/luperson/train_resolution_distribution_{timestamp}.png')
        # plot_distribution(resolutions,(0,30000) ,'Distribution of Image Resolutions', 'Resolution (pixels)', 'Frequency', f'/home/face/kaichengyang/tianluzheng/inference_llm/data_attribute/luperson/train_resolution_distribution_{timestamp}.png')
        # plot_distribution(caption_lengths,(0,500) ,'Distribution of Caption Lengths', 'Caption Length', 'Frequency', f'/home/face/kaichengyang/tianluzheng/inference_llm/data_attribute/luperson/train_caption_length_distribution_{timestamp}.png')
