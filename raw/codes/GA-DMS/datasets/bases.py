...
class FilterDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        caption_tokens = tokenize(caption, tokenizer=self.tokenizer, text_length=self.text_length, truncate=self.truncate)
        mlm_tokens, mlm_labels = self._build_random_masked_tokens_and_labels(caption_tokens.clone().cpu().numpy(), sim,0.3,1)
        nose_tokens, _ = self._build_random_masked_tokens_and_labels(caption_tokens.clone().cpu().numpy(), 1-sim,0.2,2)        
    ...
    def _build_random_masked_tokens_and_labels(self, tokens, sim,max_num,mode):
        """
        Masking some random tokens for Language Model task with probabilities as in the original BERT paper.
        :param tokens: list of int, tokenized sentence.
        :return: (list of int, list of int), masked tokens and related labels for MLM prediction
        ...
        if mode == 1:
            ...
            k = 20.0
            threshold = 0.9
        # threshold = torch.tensor(0.1).to(sim.device)  # 控制中点
        ori_pro = sigmoid(sim, k, threshold,max_num)
