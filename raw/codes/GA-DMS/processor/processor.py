...
def do_pretrain_grad(start_epoch, args, model, train_loader, evaluator0,evaluator1,evaluator2, optimizer,
    ...
    for epoch in range(start_epoch, num_epoch + 1):
        with torch.no_grad():
            if epoch % 1 == 0: 
                ...
                train_loader = build_filter_loader(args, trainset,epoch)
...
def do_pretrain(start_epoch, args, model, train_loader, evaluator0,evaluator1,evaluator2, optimizer,
    ...
    if get_rank() == 0:
        ...
        # top1 = evaluator0.eval(model.eval())# module
        # top1 = evaluator1.eval(model.eval())
    ...
    for epoch in range(start_epoch, num_epoch + 1):
        ...
        for n_iter, batch in enumerate(train_loader):
            ...
            ret,grad_emap = model(batch)

            with torch.no_grad():
                # similarity_matrix = torch.einsum('nld,nkd->nlk', [F.normalize(fu_t_feats,dim=-1), F.normalize(fu_i_feats[:,1:,:],dim=-1)])
                # similarity_matrix = similarity_matrix.max(-1)[0]
                for idx, sim in zip(batch['image_ids'].data, grad_emap):
                    trainset[idx][-1] = sim.data.cpu().numpy()
