...
def parse_img_size(size_str):
    """Parse the image size from a comma-separated string to a tuple of integers."""
    try:
        width, height = map(int, size_str.split(','))
        return (width, height)
    except ValueError as e:
        raise argparse.ArgumentTypeError(f"Invalid image size format: {size_str}. Expected format 'width,height'. Error: {e}")
...
def get_args():
    ...
    parser.add_argument("--img_size", type=parse_img_size, default=(384, 128))
