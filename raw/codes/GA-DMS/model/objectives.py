...
def compute_sdm_ddp(args,image_features, text_features, pid, logit_scale, image_id=None, factor=0.3, epsilon=1e-8):
    """
    Similarity Distribution Matching
    """
    image_features = image_features.contiguous()
    text_features = text_features.contiguous()
    all_image_features,all_text_features,all_pid = gather_features_sdm(image_features,text_features,pid,args.rank,args.world_size)
    batch_size = all_image_features.shape[0]
    all_pid = all_pid.reshape((batch_size, 1)) # make sure pid size is [batch_size, 1]
    
    pid_dist = all_pid - all_pid.t()
    labels = (pid_dist == 0).float()
    ...
    image_norm = all_image_features / all_image_features.norm(dim=1, keepdim=True)
    text_norm = all_text_features / all_text_features.norm(dim=1, keepdim=True)
...
def gather_features_sdm(image_features,text_features,pid,rank=0,world_size=1):
    '''
    收集全局特征
    '''
    ...
    gathered_pid = [torch.zeros_like(pid) for _ in range(world_size)]
    ...
    torch.distributed.all_gather(gathered_pid, pid)
    gathered_image_features[rank] = image_features
    gathered_text_features[rank] = text_features
    gathered_pid[rank] = pid
    all_image_features = torch.cat(gathered_image_features, dim=0)
    all_text_features = torch.cat(gathered_text_features, dim=0)
    all_pid = torch.cat(gathered_pid, dim=0)
    return all_image_features,all_text_features,all_pid
