    ...
    def forward(self, x, modal = 'text'):
        if self.training:
            ...
            if modal=='visual':
                ...
                x = self.resblocks(x)
                return x #torch.cat([x,x_fu1],dim=1)
            elif modal == 'text_1':
                mid_text_feature = self.resblocks[:-i](x)
                # mid_text_feature, noise_mlm_token = mix_token.chunk(2,dim=1)
                # noise_mlm_feature = self.resblocks[-i:](noise_mlm_token)
                # noise_text_feature, mlm_text_feature = noise_mlm_feature.chunk(2,dim=1)
                return mid_text_feature #noise_text_feature, mlm_text_feature #torch.cat([x,ori_token],dim=1)
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text, modal=None):
        ...
        if modal == None:
            ...
            mid_text_feature = self.transformer(x,modal)
            return mid_text_feature
        

    def forward(self, image, text,mode):
        if mode == 'mode1':
            image_features = self.encode_image(image,modal='visual')
            mid_text_features = self.encode_text(text,modal='text_1')
            return image_features,mid_text_features
