...
def build_pedes_data(config):
    ...
    test_dataset_sl = ps_eval_dataset(config, val_transform, split='test', max_words=text_length, is_sl=True)
    ...
    test_dataset_tl = ps_eval_dataset(config, val_transform, split='test', max_words=text_length, is_sl=False)
    test_loader_tl = DataLoader(
        dataset=test_dataset_tl,
        batch_size=32,
        shuffle=False,
        sampler=test_sampler,
        drop_last=False,
