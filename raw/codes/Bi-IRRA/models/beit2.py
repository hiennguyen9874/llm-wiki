...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, return_attention=False, return_qkv=False, image_atts=None, output_attentions=None, unmask_ids=None):
        B, N, C = x.shape
        qkv_bias = None
        ...
        if self.relative_position_bias_table is not None:
            ...
            if unmask_ids is not None:
                N_o = relative_position_bias.shape[2]
                N_r = unmask_ids.shape[1]
                mask_index = unmask_ids.unsqueeze(1).unsqueeze(-1).repeat(1, self.num_heads, 1, N_o)
                relative_position_bias = relative_position_bias.repeat(unmask_ids.shape[0], 1, 1, 1)
                relative_position_bias = torch.gather(relative_position_bias, dim=2, index=mask_index)
                mask_index = unmask_ids.unsqueeze(1).unsqueeze(2).repeat(1, self.num_heads, N_r, 1)
                relative_position_bias = torch.gather(relative_position_bias, dim=3, index=mask_index)
...
class Block(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, return_attention=False, return_qkv=False, image_atts=None, output_attentions=None, unmask_ids=None):
        ...
        if self.gamma_1 is None:
            ...
            y, x_attns = self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias, image_atts=image_atts, output_attentions=output_attentions, unmask_ids=unmask_ids)
            x = x + self.drop_path(self.gamma_1 * y)
            x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 use_mim=False, mim_mask_ratio=None):
        super().__init__()
    ...
    def forward(self, x, idx_to_group_img=None, image_atts=None, output_attentions=None, output_hidden_states=None,
        ...
        if is_mask:
            input_size = int(math.sqrt(seq_len))
            num_masking_patches = int(seq_len * self.mim_mask_ratio)
            # from models.masking_generator import MaskingGenerator
            # mask_generator = MaskingGenerator(input_size=input_size, num_masking_patches=num_masking_patches)
            # mask = mask_generator()
            if mask_type == 'random':
                mask = np.zeros((input_size, input_size), dtype=int)
                indices = np.arange(seq_len)
                chosen_indices = np.random.choice(indices, size=num_masking_patches, replace=False)
                mask[np.unravel_index(chosen_indices, (input_size, input_size))] = 1
            elif mask_type == 'grid':
                mask = np.indices((input_size, input_size))[1] % 2
                mask = np.where(np.arange(input_size)[:, None] % 2 == 0, mask, 1 - mask)
            ...
            mask = torch.tensor(mask, device=x.device).flatten(0).unsqueeze(1).repeat(1, 768)
            mask_token = self.mask_token.expand(batch_size, seq_len, -1)
            w = mask.unsqueeze(0).type_as(mask_token)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            x, x_attns = blk(x, rel_pos_bias=rel_pos_bias, output_attentions=output_attentions, unmask_ids=None)
    ...
    def mae_random_masking(self, x, mask_ratio):
        """
        ...
        ids_keep, ids_index = ids_shuffle[:, :len_keep].sort(dim=1)
