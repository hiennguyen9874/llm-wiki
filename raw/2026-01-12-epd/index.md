...
source: "https://www.lmsys.org/blog/2026-01-12-epd"
domain: "lmsys.org"
language: "en"
description: "We introduce Encoder-Prefill-Decode (EPD) Disaggregation in SGLang, a novel architecture that separates vision encoding from language processing in Vision-Language Models (VLMs). This can enable:  - I..."
word_count: 1239
---
...
5. **LLM Computation**: The prefill server combines vision embeddings with text tokens to form mm\_inputs containing pre-computed tensors. The LLM performs Prefill and Decode computation based on these embeddings. If PD disaggregation is enabled, the existing Prefill-Decode transfer logic is reused; otherwise, token generation happens directly on the prefill server.

### Key Components

![EPD Workflow](assets/epd_workflow.png)

![EPD Architecture](assets/epd_architecture.svg)
...
- Configurable cache size (default 4GB via SGLANG\_VLM\_CACHE\_SIZE\_MB)
...
![TTFT Results](assets/epd_vs_colocate_ttft.png)

Mean TPOT (EPD vs colocate)

![TPOT Results](assets/epd_vs_colocate_tpot.png)
...
![Throughput Results](assets/epd_vs_colocate_throughput.png)
