...
\begin{abstract}
%Learning good transferable representations proves to be crucial for many machine learning tasks. 
...
%allow for linear cross-modality operations or complex text queries to achieve many surprising image retrieval results. 
\end{abstract}
...
In the existing literature, visual %representation 
and vision-language representation learning are mostly studied separately with different training data sources. In the vision domain, %representation learning 
pre-training
...
and ImageBERT
%dataset 
\cite{qi:imagebert} require even heavier work on human annotation, semantic parsing, %analysis for
cleaning and balancing. As a result, the scales of these datasets are only in the realm of $\sim$10M examples. This is at least an order of magnitude smaller than their counterparts in the vision domain, and much smaller than large corpora of text from the internet for 
%language model 
NLP
pre-training (e.g., \citet{devlin:bert, radford:gpt2, yang:xlnet, liu:roberta, raffel:t5}).\looseness=-1

In this work, %motivated by the success of language model pre-training on large-scale noisy text corpora, %(e.g.,~\cite{devlin:bert, radford:gpt2, raffel:t5}), 
we leverage a dataset of over one billion noisy image alt-text pairs to scale visual and vision-language representation learning. 
%We trade quality for quantity and obtain our dataset by relaxing expensive filtering and post-processing steps in the Conceptual Captions dataset~\cite{sharma:cc3m}, and instead, only applying minimal frequency-based filtering. 
...
High-quality visual representations for classification or retrieval are usually pre-trained on large-scale labeled datasets \cite{mahajan:wsl, kolensnikov:bit, dosovitskiy:vit, juan:graphrise}. Recently, self-supervised \citep{chen:simclr, tian:cmc, he:moco, misra:pirl, li:pcl, grill:byol, caron:swav} and semi-supervised learning \citep{yalniz2019billion, xie:noisy_student, pham:mpl} have been studied as alternative paradigms. %Both self- and semi-supervised learning rely on advanced data augmentations. %which limits their generalizability. 
%For instance, self-supervised learning checkpoints may not transfer to other tasks~\cite{zoph2020rethinking}. Likewise, although semi-supervised learning has shown state-of-the-art performance on ImageNet~\cite{xie:noisy_student,pham:mpl}, the models trained on ImageNet may possess limitations of ImageNet itself~\cite{zoph2020rethinking}. 
%Due to this reason, models
However, models trained by these methods so far show limited transferability to downstream tasks~\cite{zoph2020rethinking}.\looseness=-1 %Moreover, semi-supervised 
%This is perhaps due to the dependence on data augmentation in self-supervised learning or careful domain matching between unlabeled  and labeled data in semi-supervised learning.

%Semi-supervised learning also needs careful domain matching between unlabeled data and labeled data~\cite{yalniz2019billion,xie:noisy_student}, which further limits their generalizability.
%To achieve comparable or stronger performance on visual classification tasks than supervised learning, semi-supervised learning also relies on large-scale external data. Although such data can be unlabeled, domain matching is very important, which still requires non-trivial data curation and is also hard to generalize to arbitrary downstream tasks.

%Another stream of work on visual representation learning leverages images and natural language captions. 
...
%been studied. Most commonly used datasets are . Among these datasets, Flickr~\cite{thomee:flickr} is much smaller than ours while COCO Captions~\cite{chen:coco} require curation and annotation. 
%In terms of methodologies, that ConvNets that learn to predict the captions from images can improve visual representations, zero-shot transfer and image retrieval, which inspires for our work. %Recently,~\citet{desai:virtex,sariyildiz2020learning,zhang2020contrastive} show that image captioning can allow for strong visual representations, albeit at smaller scales than our work.

%In terms of modeling, our method uses the contrastive loss which is similar to~\citet{zhang2020contrastive}, who apply it to the medical domain. 
...
% To further boost the retrieval quality, various architectural changes have been employed~\cite{li:vsrn,chen:vsepooling}.
%pre-trained visual representations on detected objects or uniform image grids with aggregation through attention-based models \cite{li:vsrn} or learnable pooling \cite{chen:vsepooling} are used instead of single full-image encoder. 
%While visual-semantic embedding (VSE) focuses on learning separate embeddings for image and text which can later be fused, usually via dot-product, as similarity measurement, vision-language modeling via early fusion \cite{lu:vilbert, chen:uniter} emerges following the works  in NLP like BERT \cite{devlin:bert}. %These methods typically apply cross-modal transformers on the sequences of image representation (e.g., pre-trained visual representations of detected objects) and text representations (e.g., token embeddings). 
...
% We first crawl billions of Internet webpages and collect images with English alt-text HTML attributes similar to \cite{qi:imagebert, sharma:cc3m}. Alt-text is primarily used to describe images to visitors who are unable to see them and hence is a great source of obtaining related image-text pairs. Compared to other sources like surrounding text, alt-text data is much simpler to get and requires less processing. The other possible text source of images is their web page titles, and we leave further comparison on different data sources to future work.
...
%This is two orders of magnitude larger than the previous largest dataset, Conceptual Captions~\cite{sharma:cc3m}. 
Figure \ref{fig:examples} shows some sample image-text pairs from the dataset.\looseness-1

\begin{figure}[!htb]
% google drawing link:https://docs.google.com/drawings/d/1Nt8g7qOEVxzXrXtsAHON2_oLaWMLZRgNTdSOx_uoazA/edit?usp=sharing
\begin{center}
...
% Figure \ref{fig:image_count_histogram} shows the histogram of number of associated images per text.

% \begin{figure}[ht]
% \begin{center}
%     \begin{subfigure}[b]{0.45\textwidth}
%         \centerline{\includegraphics[width=3.3in]{image_count_histogram.pdf}}
%         \vskip -0.1in
%         \caption{Histogram of number of associated images per text.}
%         \label{fig:image_count_histogram}
%     \end{subfigure}
%     \vskip 0.1in
%     \begin{subfigure}[b]{0.45\textwidth}
%         \centerline{\includegraphics[width=3.3in]{in_vocabulary_rate_histogram.pdf}}
%         \vskip -0.1in
%         \caption{Histogram of vocabulary coverage rate.}
%         \label{fig:in_vocabulary_rate_histogram}
%     \end{subfigure}
%     \vskip 0.1in
%     \begin{subfigure}[b]{0.45\textwidth}
%         \centerline{\includegraphics[width=3.3in]{text_length_histogram.pdf}}
%         \vskip -0.1in
%         \caption{Histogram of text lengths.}
%         \label{fig:text_length_histogram}
%     \end{subfigure}
%     \vskip -0.1in
%     \caption{Histograms related to minimal text-based cleaning.}
% \end{center}
% \end{figure}


% Figure \ref{fig:in_vocabulary_rate_histogram} shows the histogram of vocabulary coverage ratio. 

% Examples of alt-texts with different unigram/bigram token coverage rates can be found in Table~\ref{tab:examples_by_in_vocabulary_rate}.

% \begin{table*}[t]
% \caption{Example alt-texts of different in-vocabulaury rate buckets.}
% \label{tab:examples_by_in_vocabulary_rate}
% \vskip 0.15in
% \begin{center}
% \begin{small}
% % \begin{sc}
% \begin{tabular}{rl}
% \toprule
% In-vocab rate & \multicolumn{1}{c}{Example alt-text}                                                                \\
% \midrule
% {[}0.0, 0.4)         & image\_tid 25\&id mggqpuweqdpd\&cache 0\&lan\_code 0                                                \\
% {[}0.4, 0.8)       & shoulder bags travelcomputerbag star luggageampbag                                                  \\
% {[}0.8, 1.0)         & special friends day always good for the soul being poolside is too kindercalmer racv summer breathe \\
% = 1.0                & senior gentleman reading a newspaper and leaning against a wall stock image                         \\
% \bottomrule
% \end{tabular}
% % \end{sc}
% \end{small}
...
% Lastly, alt-texts with shorter than 3 unigrams as too general descriptions. 
% Alt-texts longer than 20 unigrams are also discarded to reduce the computation burden on the text encoder in our model. Figure \ref{fig:text_length_histogram} shows the detailed histogram.

% The numbers of image-text pairs remained after each cleaning stage are summarized in Table~\ref{tab:cleaning_stats}.

% \begin{table*}[t]
% \caption{Number of image-text pairs remained after each stage of cleaning.}
...
% \begin{tabular}{lc}
% \toprule
% \multicolumn{1}{c}{Filtering criteria} & Number of image-text pairs remained      \\
% \midrule
% Image-based filtering                  & 7.2B                                     \\
% Number of associated images $\leq$ 10  & 5.3B                                     \\
% In-vocabulary rate = 1                 & 2.4B                                     \\
% 3 $\leq$ Number of unigrams $\leq$ 20  & 1.72B                                    \\
% Deduping against test sets             & 1.72B                                    \\
% \bottomrule
% \end{tabular}
% % \end{sc}
% \end{small}
...
% As shown our data collection procedure is very simple and requires no expert knowledge. All the cleaning/filtering are purely frequency-based. With the light-weighted cleaning the final dataset contains 1.67 billions of images and 1.84 billions of total image-text pairs. Since there is no semantic-based cleaning applied and the alt-texts are not in any way restricted or enforced to be good image descriptions, the dataset can still be noisy, but we find that this large-scale dataset is effective in pre-training high-quality image and text representations. 

\section{Pre-training and Task Transfer}
\subsection{Pre-training on Noisy Image-Text Pairs}
We pre-train ALIGN using a dual-encoder architecture. The model consists of a pair of image and text encoders with a cosine-similarity combination function at the top. We use EfficientNet with global pooling (without training the 1x1 conv layer in the classification head) as the image encoder and BERT with [CLS] token embedding as the text embedding encoder (we generate 100k wordpiece vocabulary from our training dataset). A fully-connected layer with linear activation is added on top of BERT encoder to match the dimension from the image tower. Both image and text encoders are trained from scratch. %We explore different image and text backbones with the largest being EfficientNet-L2 for image and BERT-Large for text.
...
\vspace{-0.2cm}
\begin{equation}\label{eq:i2t_loss}
\small
...
\begin{equation}\label{eq:t2i_loss}
\small
...
We evaluate ALIGN models on image-to-text and text-to-image retrieval tasks, with and without finetuning. Two benchmark datasets are considered: Flickr30K \cite{plummer:flickr30k} and MSCOCO \cite{chen:coco}. %We report recall@K metrics for both retrieval tasks.
We also evaluate ALIGN on Crisscrossed Captions (CxC)~\cite{parekh:cxc}, which is an extension of MSCOCO with additional human semantic similarity judgments for caption-caption, image-image, and image-caption pairs.
...
\toprule
& \multicolumn{3}{c|}{image $\to$ text} & \multicolumn{3}{c|}{text $\to$ image} & \multicolumn{3}{c|}{text $\to$ text} & \multicolumn{3}{c}{image $\to$ image} \\
\midrule
& R@1 & R@5 & R@10 & R@1 & R@5 & R@10 & R@1 & R@5 & R@10 & R@1 & R@5 & R@10 \\
...
\centering
\small
\caption{Spearman's R Bootstrap Correlation ($\times 100$) on Crisscrossed Captions~(CxC) dataset. ALIGN is compared with VSE++~\cite{faghri:vse++}, VSRN~\cite{li:vsrn}, DE\textsubscript{I2T}~\cite{parekh:cxc}, and DE\textsubscript{T2T+I2T}~\cite{parekh:cxc}.}
...
\hline
%USE\textsubscript{mling-large} & 71.4$\pm$0.4 & -- & -- & -- \\
%ResNet-152 & --& 59.2$\pm$1.3 & -- & -- \\ 
%SimCLRv2 & -- & 74.3$\pm$0.9 & -- & -- \\ [1mm] 
VSE++                      & \textbf{74.4$\pm$0.4} & 73.3$\pm$0.9 & 55.2$\pm$1.5 & 67.6 \\
VSRN                       & 73.0$\pm$0.4 & 70.1$\pm$1.0 & 60.4$\pm$1.3 & 67.8 \\
DE\textsubscript{I2T}      & 50.9$\pm$0.6 & \textbf{81.3$\pm$0.7} & 61.6$\pm$1.4 & 64.6 \\
DE\textsubscript{T2T+I2T}  & 74.2$\pm$0.4 & 74.5$\pm$0.9 & 61.9$\pm$1.3 & 70.2 \\
\textbf{ALIGN}             & 72.9$\pm$0.4 & 77.2$\pm$0.8 & \textbf{67.6$\pm$1.2} & \textbf{72.6}\\
\bottomrule
\end{tabular}
}
\vspace{-5mm}
...
\midrule
Bit-L  & 78.72 & - & - & - \\
\textbf{ALIGN} & \textbf{79.99$\pm$0.15} & 83.38 & 87.56 & 73.25 \\
\bottomrule
\end{tabular}
}
% \end{sc}
...
%BiT-L (ResNet152~x~4) 
BiT-L& 99.63 & 96.62 & -  & - \\
%SAM-baseline (EfficientNet-L2) 
SAM-baseline
& 99.60 & 96.92 & 95.07 & 96.03 \\
%SAM-final (EfficientNet-L2) 
SAM-final
...
% Table \ref{tab:ablation_b7_l2} compares ALIGN-L2 with ALIGN-B7 (both with BERT-Large) on MSCOCO image-text retrieval and ImageNet classification results, showing additional quality gain from B7 to L2.

% [Comment] Worth highlighting that we are not saturated, but argument isn't strong without L2 result in ablation.
...
% (+BERT-Large) & image$\to$text R@1 & text$\to$image R@1 & image$\to$text R@1 & text$\to$image R@1 & \textbf{(fine-tuned)}\\
% \midrule
...
%\footnote{A fully-connected layer with linear activation is used for dimension mapping.}
...
%, which also suggest that a potential improvement can be obtained by keep increasing the dimension size.
% Note that while 640 is already larger than the EfficientNet-B5 feature dimension (512), it is still smaller than BERT-base feature dimension (768).
Rows 5 and 6 show that using fewer in-batch negatives (50\% and 25\%) in the softmax loss will degrade the performance.
%By default we use all non-matched image-text pairs in a batch as negatives. In this ablation, we show that  will degrade the performance.
%While using more negatives is important for pushing the limit of the model quality, reasonable quality can be achieved with fewer negatives. This shows that without considering training wall time, a model trained with fewer parallel computing cores (compared to the 512 Cloud TPUv3 cores we use in training ALIGN) is also acceptable.
...
% \textbf{\textcolor{red}{Placeholder: Tentatively Add fine-grained classification tasks (flowers/pets/cars/food)}}

%\section{Cross-modal Search using ALIGN}
%\section{Cross-modal Search using ALIGN and Emergent Linear Relationships between Image and Text Embeddings}
%\vspace{-3mm}
...
\begin{figure}[!htb]
% google drawing link:https://docs.google.com/drawings/d/1PFTTziBPss60PNcPhFoZUJSrT3ka9DgpQ9VjMglRPRM/edit?usp=sharing&resourcekey=0-qsNFndHb369WnHyR0pOJrw
\vspace{-1mm}
\begin{center}
...
\begin{figure}[h!]
% google drawing link:https://docs.google.com/drawings/d/1IglQ6DWekUZ5bY4qDH4gFBDjdVyAoe_gcFIj4T7WAS0/edit?usp=sharing&resourcekey=0-5oWZauwBZRsQXvExuRzdZA
\begin{center}
    \centerline{\includegraphics[width=\linewidth]{NNIQ_i+t.pdf}}
    \vskip -0.1in
    % \caption{Image retrieval with image$\pm$text queries. We add (or subtract) text query embedding to (or from) the image query embedding, and then use the resulting embedding to retrieve relevant images using cosine similarity.}
    \caption{Image retrieval with image$\pm$text queries. We add (or subtract) %normalized 
    text query embedding
    %(with a scaling factor of 2) 
    to (or from) the %normalized 
    image query embedding, and then use the resulting embedding to retrieve relevant images using cosine similarity.}
    \label{fig:nniq_i+t}
\end{center}
\vspace{-10mm}
\end{figure}
...
\begin{table}[h]
\begin{center}
\caption{SimLex-999 results (Spearman’s $\rho$).}
\label{tab:simlex}.
\begin{small}
% \begin{sc}
\begin{tabular}{l|rrr}
