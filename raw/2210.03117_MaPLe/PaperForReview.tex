...
\def\cvprPaperID{7620} % *** Enter the CVPR Paper ID here
\def\confName{CVPR}
\def\confYear{2023}
...
The combined use of language supervision with natural images is found to be of great interest in the computer vision community. In contrast to models learned with only image supervision, these vision-language (V-L) models encode rich %language-driven visual 
multimodal representations. Recently, V-L models like CLIP \cite{radford2021learning}, ALIGN \cite{jia2021scaling}, LiT \cite{zhai2022lit}, FILIP \cite{yao2021filip} and Florence \cite{yuan2021florence} have demonstrated exceptional performance on a wide spectrum of tasks including few-shot and zero-shot visual recognition. These models learn joint image-language representations in a self-supervised manner using abundantly available data from the web.
...
% UPL~\cite{huang2022unsupervised} optimizes the language prompts in an unsupervised fashion.
...
% \noindent We conduct experiments in three benchmark settings for image recognition.
...
\begin{table}[t!]
    ...
    Method  & Base Acc. & Novel Acc. & HM & GFLOPS\\
            \midrule
    % CoOp\cite{zhou2022learning} (IJCV) & 82.69 & 63.22 & 71.66 & 166.8\\
    % Co-CoOp\cite{zhou2022conditional} (CVPR'22) & 80.47 & 71.69 & 75.83 & 166.8\\
    % \midrule
...
%     \midrule
%     \rowcolor{tabhighlight}
%     4: MaPLe (Ours)  &	\textbf{{82.28}} &	\textbf{75.14} &	\textbf{78.55} & 167.0\\
%     \bottomrule
%     \end{tabular}
%     }}
%     \caption{ Comparison of MaPLe with different prompting designs in base-to-novel generalization. Results are averaged over 11 datasets. HM refers to harmonic mean. 
...
\begin{itemize}
    ...
    % \item Prompting complexity (Appendix~\ref{appendix:prompt_complexity})
...
% initializing prompts at different layers with the same template
...
% the latter approach provides lesser gains as compared to our default approach and 
...
% We observe significant gains in case of datasets that have distribution different to that of the CLIP pretraining dataset, such as EuroSAT, FGVCAircrafts and DTD, as compared to generic-objects datasets. Fig.~\ref{fig:per_class_bar} shows the analysis of per class accuracy for selected datasets in the order of increasing diversity (distribution gap w.r.t CLIP pretraining dataset). The figure indicates that multi-modal prompting in MaPLe achieves better gains than Co-CoOp as the diversity of dataset increases. Similar trends have been observed in literature \citep{bahng2022visual}.

% We observe that MaPLe provides significant gains over Co-CoOp for vision concepts that are rare and less generic, e.g satellite images. In contrast, MaPLe performs competitively to Co-CoOp on frequent and more generic categories e.g forest, river, dog, etc. Multi-modal prompts allow MaPLe to better adapt CLIP for datasets and visual concepts that are rare and not commonly occurring as compared to existing uni-modal prompting techniques.

\label{appendix:additional_analysis}

% \section{Prompting Complexity}\label{appendix:prompt_complexity}
% Table~\ref{tab:complexity} shows the computational complexity of MaPLe in comparison with other approaches. Although MaPLe utilizes multi-modal prompts, its overall FLOPS (Floating Point Operations) exceeds only by 0.1\% over CoOp and Co-CoOp. The independent V-L prompting also provides comparable FLOP count. In terms of inference speed, Co-CoOp is significantly slower and the FPS (Frames Per Second) remains constant as the batch size increases. In contrast, MaPLe has no such overhead and provides much better inference and training speeds. Further, MaPLe provides better convergence as it requires only half training epochs as compared to Co-CoOp (5 vs 10 epochs). 
...
%     \toprule
%     \multirow{2}{*}{Method} & \multirow{2}{*}{GFLOPS} & {\% GFLOPS} & \multicolumn{3}{c}{FPS} & \multirow{2}{*}{BS Overhead} \\ 
%     & & w.r.t Co-CoOp & BS=1 & BS=4 & BS=100 & \\
%     \midrule
...
%     \bottomrule
%     \end{tabular}
%         \caption{Comparison of computational complexity among different prompting methods. 
%     % MaPLe shows FLOP count comparable with existing prompting methods CoOp and Co-CoOp, while it significantly boosts the training and inference speed as compared to instance conditioning based Co-CoOp method. FLOP analysis is performed using a single image-text pair.
%     }
% \end{table}
...
% We perform an experiment to validate that performance gain of MaPLe is not mainly attributed towards its increased learnable parameters. 
% As shown in Table~\ref{table:heavier_Co-CoOp}, 
...
% The results shows that the performance of heavier Co-CoOp does not improves significantly in comparison to the standard Co-CoOp. This suggests that only increasing the learning capacity of model does not aid in overall improvement, whereas MaPLe approach which adapts both vision-language branches of CLIP together is crucial for enhanced performance.
\begin{table}[h]
\tablestyle{6pt}{1.1}
\addtolength{\tabcolsep}{-1.5pt}
\centering
...
% to explore if the parameter count is the source of gain.
} 
