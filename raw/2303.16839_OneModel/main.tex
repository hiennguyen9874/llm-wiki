...
% \title{A simple Joint Learning for MultiModal tasks}
% \title{A Simple Architecture for Joint Learning for MultiModal Tasks}
%\title{MaMMUT: A Simple Architecture for Joint Pre-Training for MultiModal Tasks}
...
\vspace{-1cm}
% \author{\name Weicheng Kuo$^*$ \email weicheng@google.com 
%       \AND \name AJ Piergiovanni$^*$ \email ajpiergi@google.com 
%       \AND \name Dahun Kim$^\dagger$ \email 
%       \AND \name Xiyang Luo$^\dagger$ \email
%       \AND \name Ben Caine \email 
%       \AND \name Wei Li \email 
%       \AND \name Abhijit Ogale \email 
%       \AND \name Luowei Zhou \email 
%       \AND \name Andrew Dai \email 
%       \AND \name Zhifeng Chen \email 
%       \AND \name Claire Cui \email  
%       \AND \name Anelia Angelova \email anelia@google.com \\
%       \newline
% Google Research      
% }
...
\def\openreview{\url{https://openreview.net/forum?id=FqOG4osY7C}} % Insert correct link to OpenReview for camera-ready version
...
Some vision-language pre-training approaches apply a contrastive loss, in a dual-encoder style architecture, e.g. CLIP, Align, Florence~\citep{radford2021clip,align,yuan2021florence}. 
...
losses from language learning~\citep{pmlr-v139-cho21a,pali,piergiovanni2022answer,wang2021simvlm,omniVL}. Architectures with cross-attention over frozen or partly frozen language models have also been popular~\citep{flamingo}.
...
This provides a simpler alternative to previous approaches~\citep{ albef,yu2022coca,singh2022flava,yuan2021florence}, where our model architecture is more compact and shared more broadly across tasks in the model. % and further allows for language-only decoder reuse.
Furthermore, our model allows us to apply this architecture to video by a seamless adaptation based on the TubeViT approach~\citep{piergiovanni2022tubevit}. This allows us to address video-text tasks, such as Video Question Answering (VideoQA) and Video Captioning successfully,
...
In comparison, many of the prior approaches, despite reporting results on 10, 20 or more tasks or training bigger models ~\citep{pali,singh2022flava,flamingo,yuan2021florence}, accommodate fewer categories of tasks than our model does. One of the largest models, PaLI~\citep{pali} addresses only classification, VQA and image captioning task categories, but is not designed to handle retrieval, detection, %, which are better suited to the dual-encoder architectures~\citep{radford2021clip,align}, 
or video tasks. 
 GIT~\citep{wang2022git} extends the model to videos, % processing a single frame at a time, 
 but does not handle retrieval or detection.
FLAVA~\citep{singh2022flava}, %or object localization tasks. 
Florence~\citep{yuan2021florence}, ALBEF~\citep{albef}, BLIP~\citep{li2022blip} which are based on constrastive learning,  accommodate more tasks, %significant downstream adaptations are needed for VQA-like tasks.% are needed to extend the task offering. 
but do not perform video-language or object localization tasks, or need significant downstream adaptations to do so~\citep{yuan2021florence}.
Flamingo~\citep{flamingo} %, which incorporates video and image tasks by design, 
is not able to do retrieval or object detection.
...
Extensions to these approaches, where masking is done in the image space~\citep{bao2021beit}, have also been considered. 
...
%, propose to start from the same core model, but re-configure it according to the type of task.
%However this approach requires significant changes of the model depending on the task.
 ...
 A number of video foundational models have been proposed~\citep{merlot,fu2021violet,flamingo,internvideo,omniVL,VINDLU,UniVL}. The Flamingo model~\citep{flamingo} extends a large frozen language model, with image and video inputs to deliver impressive results. % over a range of vision-language tasks.
 VIOLET~\citep{fu2021violet} uses masked language and masked-video modeling for joint video-text learning. 
...
We encode the images into latent representation using a neural network encoder (such as ViT~\citep{dosovitskiy2020image}). The texts are both encoded and decoded with a Transformer-based decoder-only model.
...
In contrast to other architectures, Figure~\ref{fig:main_arch}, middle and right, we use a single vision encoder and a single text decoder. Unlike encoder-decoder models~\citep{pali,wang2022git,wang2022image} and contrastive alignment and captioning~\citep{albef,yu2022coca}, this is simpler and more flexible architecture. While using an encoder-decoder is popular, it comes with some disadvantages. For example, a pretrained visual encoder is  required, and contrastive tasks are usually not feasible. %, and adaptation to video is on a per-frame basis. % Additionally often an encoder-decoder is trained for text is adapted to an additional input.   
...
%moved below
%In comparison, previous approaches some serving as Video Foundational Models, exhibit large complexity. InternVideo~\citep{internvideo} for example, supports a video masked encoder for MAE style losses in addition to an additional Video/Image Encoder and a Text decoder which are trained in the Align-Before-Fuse or Contrastive Captioner style, but for videos. Flamingo~\citep{flamingo} similarly is able to process either image and video inputs, but it trains a separate image-encoder, the text encoder/decoder part of which is discarded when integrating in the main model. We note that we outperform both in our experiments on video tasks.  
...
% L_{\text{I2T}} = -{1 \over {B}} \sum_{i=1}^{B} \log({\text{exp}(v_{i}l_{i} / \tau) \over { \sum_{ji=1}^{B} \text{exp}(v_{i} l_{j} / \tau)  }}). \quad L_{\text{T2I}} = -{1 \over {B}} \sum_{i=1}^{B} \log({\text{exp}(v_{i}l_{i} / \tau) \over { \sum_{j=1}^{B} \text{exp}(v_{ji} l_{ij} / \tau)  }}).
% \end{equation}
...
Our model consists of a standard ViT-Huge~\citep{dosovitskiy2020image,ScalingViTs} image encoder (of 650M parameters) and a transformer text decoder of 1B parameters. %(18 layers, model dimension 1536, hidden dimension 12288).   
The cross attention layers are applied every two decoder layers, where the model and hidden dimension follow the text decoder (420M params). Ablation results are conducted with ViT-Base image encoder and a smaller text decoder using 128M parameters, % (8 layers and model dimension 1024, hidden dimension 4096), 
...
%In comparison to prior work, e.g.VindLU~\citep{VINDLU} the adaptation needs both changes in the main model, as well as thee additional video-text learning losses. 

We note that this change does not need any additional video data pre-training, and all experiments conducted in the paper are done by directly fine-tuning of the \ours image-text model on a video dataset. % We also compare adding further video training (e.g. VideoCC-3M \citep{}).

%Similarly, OmniVL~\citep{omniVL}...
...
% We contrast our architecture (Figure~\ref{fig:main_arch}) with popular encoder-decoder architectures and contrastive captioner~\citep{}. The encoder-decoder architectures are not able to perform retrieval tasks; the two-tower architectures~\citep{clip,align} are designed for cross-modal retrieval (image-to-text or text-to-image) but do not have the ability to generate text i.e. captioning, visual question answering; the constrastive captioner~\citep{coca} and TODO~\citep{} (there was another similar work) combine these two concepts, using an encode-decodr architecture where it combined the text decoder in a two tower architecture~\citep{align} to apply constractive loss.
...
% Previous VL approaches focus on training using the global information from the image, without consideration for object detection tasks. Detection is trained / fine-tuned subsequently, adapting the features to localization specific outputs. In some cases such training provides powerful features which can be incorporated in downstream detection e.g. ~\citep{F-VLM}. 
...
%
We note that these are challenging benchmarks many approaches have been evaluated on. % We omit earlier methods to save space. 
...
%Finally, \ours achieves similar accuracy to strong closed-vocabulary approaches such as SimVLM~\citep{wang2021simvlm} and Florence~\citep{yuan2021florence}.
...
%Dynamic Pretr.~\citep{dynpretr} & 79.9 & - \\
...
Similarly, to the results on VQA, here too, models such as PaLI~\citep{pali} and GIT/GIT2~\citep{wang2022git} outperform our method.
 ...
 %LEMON-Base~\citep{lemon} &133.3 &40.3\\
 ...
 %OFA~\citep{wang2022unifying} &145.3 & \\
...
% %\shline
% %M-PLUG2~\citep{mPLUG-2}	&48.0  &58.1 & \\
% \hline
...
\begin{table}[h]
    ...
    \begin{subtable}[h]{0.4\textwidth}
        ...
        \vspace{2mm}
        \caption{\textbf{Video Captioning Results.} %Results on the Video Captioning task. 
        \ours performs well on both MSRVTT and MSVD Video Captioning Benchmarks, outperforming SOTA on MSVD by large margins. CIDEr scores are shown. As for VideoQA experiments, we use only image-language pre-training, and directly fine-tune the model on each dataset.}
        \label{tab:video_cap}
     \end{subtable}
...
% more B@4 % C from GIT paper  MSVD
%PickNet 52.3 76.5
%GRU-EVE 47.9 78.1
%SAAT 46.5 81.0
%MGSA 53.4 86.7
...
%
The open-vocabulary detector is initialized with the pretrained ViT backbone during finetuning. %, as in~\citep{kuo2022fvlm,rovit}. 
...
%RO-ViT~\citep{rovit} & 32.1 & 34.0 \\
%F-VLM~\citep{kuo2022fvlm} & 32.8 &34.9 \\
\hline
...
Effects of model scale on vision-language tasks have been explored in prior works~\citep{flamingo,pali}. We present the results of scaling image encoder in Table~\ref{tab:ablation_scale}, where we confirm increasing the capacity of image encoder yields consistent improvement. We note that text encoder size of the last row has 1B parameters as opposed to 128M in the rest of the table. We use 16K batch size in this ablation.   %Furthermore, we observe that even small models very competitive with larger SOTA models. This is due to the benefits our our approach (for direct comparison, we have used the same dataset~\citep{align} as SOTA methods~\citep{yu2022coca}.). 
...
% Method & CoCa & PaLI & Flamingo & GIT-2 & \ours(Ours) \\ 
% \hline
% PF-days & 743 & 453 & 1381 & 5513 & 134    \\
% Ratio to ours & 5.5$\times$ & 3.4$\times$ & 10.3$\times$ & 41.2$\times$ & 1$\times$  \\
% \hline
% % \bottomrule
% \end{tabular}
% \caption{\textbf{Total Compute Comparison.} We compare the total train compute use with existing foundational models. \ours is significantly cheaper to train than existing foundational models without using any pretrained models.}
% \label{tab:ablation_pretr}
% \vspace{-2mm}
% \end{table}
