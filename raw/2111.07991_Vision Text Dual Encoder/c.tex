...
\usepackage{longtable} % for multipage VTAB table
...
\def\cvprPaperID{10338} % *** Enter the CVPR Paper ID here
\def\confName{CVPR}
\def\confYear{2022}
...
\author{\normalsize{Xiaohua Zhai$^{\star\dagger}$ \authsep Xiao Wang$^{\star}$ \authsep Basil Mustafa$^{\star}$ \authsep Andreas Steiner$^{\star}$ \authsep Daniel Keysers \authsep Alexander Kolesnikov \authsep Lucas Beyer$^{\star\dagger}$}\\
Google Research, Brain Team, Zürich}
\maketitle
{\let\thefootnote\relax\footnote{
{$^{\star}$equal technical contribution, $^{\dagger}$equal advising}}}
...
Both towers are trained to minimize a contrastive loss, which encourages representations of paired images and texts to be similar, and %
%, conversely, 
...
which just teaches a text model to read out suitable representations from a pre-trained image model. % for new tasks.
\lit{} achieves better results compared with the from-scratch CLIP~\cite{clip} or ALIGN~\cite{align} models.
...
We use $32$k batch size, and tune for $18$ billion image-text pairs seen (roughly $550$k steps). See Appendix~\ref{appendix:our_details} for details.
...
We train each setting many times on the YFCC100m-CLIP dataset, varying the total number of steps from $2\,500$ to $60\,000$ in order to understand the setting's trajectory, and sweeping over learning-rates and weight-decays to avoid being misled.
...
\begin{figure}[t]
    ...
    This reveals how the different settings behave, see text for details.%further interpretation.
    }
...
\begin{table}[t]
  ...
  \begin{tabularx}{\linewidth}{p{2cm}p{0.1cm}Cp{0.1cm}Cp{0.1cm}Cp{0.1cm}Cp{0.1cm}Cp{0.1cm}Cp{0.1cm}C}
    \toprule[1pt]
     ...
     {\centering \rotatebox{90}{I$\rightarrow$T}} &&
     {\centering \rotatebox{90}{T$\rightarrow$I}}\\
    \midrule
...
\begin{table}[t]
  ...
  \begin{tabularx}{\linewidth}{p{0.4cm}p{0.1cm}p{1.2cm}p{0.1cm}p{0.5cm}p{0.1cm}Cp{0.1cm}Cp{0.1cm}C}
    \toprule[1pt]
     && \textbf{Model} && \textbf{Tok} && \textbf{INet 0shot} && \textbf{I$\rightarrow$T} && \textbf{T$\rightarrow$I} \\
    \midrule
...
% The additional improvement when using pre-trained weights is given in parentheses.
...
\begin{table}[tb]
  ...
  \begin{tabularx}{\linewidth}{p{1.5cm}p{0.1cm}Cp{0.1cm}Cp{0.1cm}Cp{0.1cm}Cp{0.1cm}C}
    \toprule[1pt]
     \bf{Dedup} && \bf{\#tune} && \bf{\#eval} && \bf{ImgNet} && \bf{I$\rightarrow$T} && \bf{T$\rightarrow$I}\\
    \midrule
     - && 0 && 0 && 70.2 && 43.6 && 28.4 \\
     test && 2.6M && 76K && 70.2 && 43.3 && 28.3 \\  % 2_575_200
     train+test && 3.6M && 220K && 69.9 && 43.7 && 28.4 \\  % 3_637_630
    \bottomrule[1pt]
  \end{tabularx}
...
Second, we use the Wikipedia based Image Text (WIT) dataset~\cite{wit_dataset} to perform T~$\rightarrow$~I retrieval across more than a hundred languages. Figure~\ref{fig:xlang_wit} gives a summary of this evaluation; a more detailed variant is provided in Appendix~\ref{appendix:multilingual_details}.
